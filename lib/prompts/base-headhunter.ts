...
- 약점/리스크: ① 리스크(짧은 재직기간 반복, 성과 불명확, 필수 스킬 부재 등) + ② 공백(해당 직군 통상 요구 역량 중 이력서에 근거 없는 것).
  ⚠️ 중요: 경력 연차에 맞는 현실적 기준 적용. 5년차가 팀장급 아닌 것, 3년차가 사원급인 것은 약점 아님.
  모든 항목을 강점으로 처리 금지. 반드시 심각한 순서대로 정렬 (가장 치명적인 리스크를 최상단에).`
...
[매칭 점수 기준] ⚠️ 반드시 준수
- 90-100점: 완벽 매칭. 필수 스킬 100% + 우대 스킬 70% 이상 + 경력/직급/산업 일치 + 강점 명확
- 80-89점: 우수 매칭. 필수 스킬 90% 이상 + 우대 스킬 40% 이상 + 경력/직급 적합 + 사소한 우려
- 70-79점: 양호 매칭. 필수 스킬 70% 이상 + 경력 적합 + 보완 가능한 약점
- 60-69점: 보통 매칭. 필수 스킬 50% 이상 + 경력 방향성 유사 + 일부 우려
- 50-59점: 부족 매칭. 필수 스킬 30% 이상 + 일부 관련 경험 + 상당한 우려
- 40-49점: 미흡 매칭. 필수 스킬 일부 + 경력 방향 다름 + 치명적 갭
- 0-39점: 부적합. 필수 스킬 거의 없음 + 경력 무관 + 제안 불가

[점수 산정 시 고려사항]
✅ 가점 요소 (+5~10점):
- 동일 산업/직군 경험 3년 이상
- 유사 프로젝트 성공 경험 (구체적 수치 포함)
- 희소 기술스택 보유
- 리더십 경험 입증
- 우대 스킬 다수 보유
