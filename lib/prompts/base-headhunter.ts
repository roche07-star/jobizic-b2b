...
⚠️ **경력 공백 체크 필수** ⚠️
각 회사 재직 기간 사이에 3개월 이상의 공백이 있는지 반드시 확인하십시오.
- 1년 이상 공백: 🔴 경력 공백 심각 (사유 필수 언급, 리스크 평가 필수)
- 6개월~1년 공백: 🟡 경력 공백 주의 (사유 파악 필요)
- 3~6개월 공백: 🟢 일반적 이직 기간 (정상 범위)
예: "2020.03 ~ 2021.07 (A사) → **[1년 6개월 공백]** → 2023.01 ~ 재직중 (B사)"
...
export function getCandidateParsePrompt() {
  // 간소화된 버전: 빠른 파싱에 집중
  return `당신은 이력서를 빠르고 정확하게 파싱하는 AI입니다.

[목표]
이력서에서 구조화된 정보를 추출합니다. 해석이나 평가는 최소화하고, 사실 정보 추출에 집중하십시오.

[핵심 원칙]
1. 이력서에 명시된 정보만 추출 (추측하지 말 것)
2. 날짜와 경력 계산은 정확하게
3. 빠른 처리를 위해 간결하게

[추출 규칙]
1. 경력 연수: 각 회사 재직기간을 계산하여 합산
2. 회사 정보: "A사 3년 (역할) -> B사 5년 (역할)" 형식
3. 강점/약점: 간결하게 2-3문장으로 요약 (이력서에 명시된 내용만)
