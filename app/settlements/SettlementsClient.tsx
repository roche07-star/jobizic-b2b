...
export default function SettlementsClient() {
  ...
  const loadSettlementGoals = async () => {
    ...
    try {
      const params = new URLSearchParams({ user_email: profile.email })
  ...
  const saveSettlementGoals = async (yearGoals: { goalAmount: number; carryover: number }) => {
    ...
    try {
      ...
      const saveRes = await fetch(`/api/goals?${params}`, {
        method: 'POST',
      ...
      if (profile?.email && typeof window !== 'undefined') {
        localStorage.setItem(`settlements_goalAmount_${profile.email}_${selectedYear}`, String(yearGoals.goalAmount))
        localStorage.setItem(`settlements_carryover_${profile.email}_${selectedYear}`, String(yearGoals.carryover))
      }
    } catch (err) {
              ...
              <tbody>
                {(() => {
                  ...
                  return settlements.map((s, idx) => {
                    ...
                    // 요율 계산
                    const ir = threshold > 0 ? (alreadyConverted ? 100 : s.incentive_rate) : s.incentive_rate

                    // 인센티브 계산
                    let incentive, seventyPart, hundredPart, incentiveSeventyPart, incentiveHundredPart
                    if (willConvert) {
                      // 전환 시 분할 계산
                      seventyPart = f(threshold - cumPersonal) // 70% 구간
                      hundredPart = f(personal - seventyPart) // 100% 구간
                      incentiveSeventyPart = f(seventyPart * s.incentive_rate / 100) // 70%
                      incentiveHundredPart = f(hundredPart * 100 / 100) // 100%
                      incentive = f(incentiveSeventyPart + incentiveHundredPart)
                    } else {
                      incentive = f(personal * ir / 100)
                    }
