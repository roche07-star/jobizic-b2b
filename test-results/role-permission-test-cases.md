...
- **예상**: NOT IMPLEMENTED
- **실제**: ❌ NOT IMPLEMENTED
- **이슈**: 중복 후보 탐지 로직 없음, DB unique constraint도 없음 
...
**⚠️ PARTIAL (개선 필요) - 2건**
1. **TC-3.2-003**: 타인 JD 수정·삭제 - UI에서 소유권 체크 없음
   - 현재: 모든 JD에 수정/삭제 버튼 표시
   - 개선: JD 목록에서 created_by === userEmail 체크 후 버튼 표시
   
2. **TC-3.3-005**: 후보자 소유권 이전 - 개별 이전 UI 없음
   - 현재: 사용자 비활성화 시 벌크 이관만 가능
   - 개선: 후보자 상세 페이지에 "소유권 이전" 버튼 추가

**❌ NOT IMPLEMENTED (명세서 기능) - 4건**
